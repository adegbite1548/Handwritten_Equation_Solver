...
class Attention(nn.Module):
    ...
    def forward(self, encoder_features, decoder_hidden, coverage):
        ...
        # Update the running coverage vector for the next time step
...
class WAPDecoderCAN(nn.Module):
    ...
    def forward(self, x, decoder_hidden, encoder_features, coverage, counting_preds):
        ...
        # Concatenate the embedding, context vector, AND the global counting predictions
        gru_input = torch.cat([embedded, context, counting_preds], dim=1)
