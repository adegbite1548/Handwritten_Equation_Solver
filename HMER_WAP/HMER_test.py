...
with torch.no_grad():
    for batch_idx, (images, targets) in enumerate(test_loader):
        ...
        for t in range(MAX_DECODE_LEN):
            # Standard WAP decoding (no current_counts passed in)
            predictions, decoder_hidden, alpha, coverage = decoder(
                decoder_input, decoder_hidden, encoder_features_seq, coverage
            )
            ...
            #Stop early if all sequences have generated <EOS>
...
# --- 6. Results Summary ---
print("\n" + "="*50)
print(f"WAP BASELINE TEST SET EVALUATION ({total_samples} samples)")
print("="*50)
